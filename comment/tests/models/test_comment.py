...
class TestComment(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create(username = 'razoan' , password = 'razoan')
        self.content_type = ContentType.objects.get_for_model(model = Discussion)
        self.discussion = Discussion.objects.create(user = self.user , title = "test" , short_description = "test" , detail = "test")
    ...
    def test_str_method(self):
        self.assertEqual(f"Comment by {self.user} on {self.content_type}" , "Comment by razoan on Discussion | discussion")
